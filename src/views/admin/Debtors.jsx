...
export default function Debtors({ onBack }) {
  ...
  // --- SECURE MULTI-TENANT DEBT DATA RETRIEVAL ---
  const fetchDebtors = useCallback(async (profile, userIsAdmin) => {
    ...
    try {
      ...
      // SECURITY SEGREGATION: Restrict regular staff view to their assigned branch location
      if (!targetIsAdmin) {
        if (targetProfile?.branch_id) {
          query = query.eq('branch_id', targetProfile.branch_id);
        } else {
          // Prevent accidental cross-leakage if location metadata is completely unmapped
          setDebtList([]);
          return;
        }
      }
      ...
      console.error("Secure debt ledger query failed:", err.message);
    } finally {
      setLoading(false);
    }
  }, [userProfile, isAdmin]);

  // --- SECURE IDENTITY CORRIDOR HANDSHAKE ---
  const checkUserAccess = useCallback(async () => {
    try {
      ...
      // STRICT SECURE DETERMINATION: Avoid dangerous fallbacks that default privileges to true
      const userIsAdmin = profile?.role === 'admin' || user.email?.includes('admin');
      setIsAdmin(userIsAdmin);

      // Trigger fetch instantly with fresh evaluation states
      await fetchDebtors(profile, userIsAdmin);
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-6 pb-24 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          <button onClick={onBack} className="text-blue-600 font-black uppercase text-xs tracking-widest hover:opacity-80 transition-all">
            {t('back')}
          </button>
          <div className="text-right">
            ...
            <p className="text-[10px] font-black text-red-500 uppercase tracking-widest">
              {isAdmin ? '👑 Global Consolidated Registry' : '📍 Branch Workspace Registry'}
            </p>
          </div>
        </div>
            ...
            <div key={profile.name} className="bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-slate-100 transition-all hover:shadow-md">
              <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4 mb-6">
                ...
                <div className="sm:text-right">
                  ...
                  <p className="text-3xl font-black text-red-600 tracking-tighter">
                    {profile.totalOwed.toLocaleString()} <span className="text-sm font-bold text-slate-400">FCFA</span>
                  </p>
                </div>
              </div>
              ...
              {isAdmin ? (
                <button 
                  ...
                  disabled={loading}
                  className="w-full bg-[#10b981] text-white py-4 rounded-[20px] font-black uppercase text-xs tracking-widest shadow-md hover:bg-emerald-600 active:scale-[0.98] transition-all disabled:opacity-50"
                >
                  {loading ? (t('reconciling_accounts') || 'Reconciling Accounts...') : (t('clear_total_balance_btn') || 'Clear Total Balance ✅')}
                </button>
              ) : (
                <div className="w-full bg-slate-50 text-slate-400 border border-slate-100/80 py-4 rounded-[20px] font-black text-center text-[10px] uppercase tracking-widest">
